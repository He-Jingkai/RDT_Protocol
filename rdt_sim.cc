...
class EventChain {
 ...
 public:
  ...
  void cancel(Event *e) {
    Event **ppcur = &head;
    while ((*ppcur != NULL) && (*ppcur != e)) ppcur = &((*ppcur)->next);
...
static double myrandom() { return (rand() * 1.0 / RAND_MAX); }
...
double GetSimulationTime() { return sim_core.time(); }
...
void Sender_StartTimer(double timeout) {
  if (tracing_level >= 1)
    fprintf(stdout,
            "Time %.2fs (Sender): the timer is started (expires at %.2fs).\n",
...
bool Sender_isTimerSet() { return (sender_timer != NULL); }
...
void Receiver_ToUpperLayer(struct message *msg) {
  ...
  for (int i = 0; i < msg->size; i++) {
    ...
    if (tracing_level >= 2) fputc(msg->data[i], stdout);
  }
...
int main(int argc, char *argv[]) {
  if (argc != 8) {
    fprintf(stderr,
            "usage: %s <sim_time> <mean_msg_arrivalint> <mean_msg_size> "
  ...
  for (int i = 0; i < 1000; i++) randtest_sum += myrandom();
  ...
  for (;;) {
    ...
    switch (e->event_type) {
      case EVENT_SENDER_FROMUPPERLAYER: {
        if (tracing_level >= 1) {
          fprintf(stdout,
                  "Time %.2fs (Sender): the upper layer instructs rdt layer to "
                  "send out a message.\n",
                  sim_core.time());
        }
      ...
      case EVENT_SENDER_FROMLOWERLAYER: {
        if (tracing_level >= 1) {
          fprintf(stdout,
                  "Time %.2fs (Sender): the lower layer informs the rdt layer "
                  "that a packet is received from the link.\n",
                  sim_core.time());
        }
      ...
      case EVENT_RECEIVER_FROMLOWERLAYER: {
        if (tracing_level >= 1) {
          fprintf(stdout,
                  "Time %.2fs (Receiver): the lower layer informs the rdt "
  ...
  fprintf(stdout, "\n");
  fprintf(stdout,
          "## Simulation completed at time %.2fs with\n"
          "\t%d characters sent\n"
          "\t%d characters delivered\n"
          "\t%d packets passed between the sender and the receiver\n",
          sim_core.time(), tot_chars_sent, tot_chars_delivered,
          tot_pkts_passed);

  if (message_verfication_passed && (tot_chars_sent == tot_chars_delivered))
    fprintf(stdout,
            "## Congratulations! This session is error-free, loss-free, and in "
            "order.\n");
  else
    fprintf(stdout,
            "## Something is wrong! This session is NOT error-free, loss-free, "
            "and in order.\n");
